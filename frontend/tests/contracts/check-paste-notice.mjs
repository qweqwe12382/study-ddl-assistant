...
assert.equal(pasteNoticeFilename('  '), '课程通知.txt')
...
assert.match(materialsSource, /materialsApi\.upload\(\[file\], null, '课程通知', notice\.sourceTime\)/)
...
assert.match(materialsSource, /const ExtractionCandidatesEditor = defineAsyncComponent\(\(\) => import\('\.\.\/components\/ExtractionCandidatesEditor\.vue'\)\)/)
assert.match(materialsSource, /<ExtractionCandidatesEditor :tasks="extractionTasks"\s*\/>/)
assert.match(materialsSource, /<el-table :data="extractionTasks"/)
