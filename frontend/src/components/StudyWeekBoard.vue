<template>
  <section class="study-week-board" :aria-busy="loading">
    <header class="board-header">
      <div>
        <p class="board-kicker">近期复习 / STUDY TRACK</p>
      ...
      <dl v-if="plan" class="plan-facts" aria-label="计划信息">
        ...
        <div v-if="plan.daily_minutes"><dt>每天</dt><dd>{{ plan.daily_minutes }} 分钟</dd></div>
      </dl>
    </header>
    ...
    <template v-else>
      ...
      <div v-else-if="!displayItems.length" class="board-empty" role="status">
           {{ delayedOnly ? '没有待完成的延期复习项。' : '这份计划还没有可展示的复习项。' }}
      </div>
      ...
      <template v-else>
        <section v-if="nextAction" class="next-action" aria-labelledby="next-action-heading">
          ...
          >
            <div class="week-rail" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div>
            <div class="next-copy">
...
<style scoped>
.study-week-board { min-width: 0; padding: 20px; color: var(--ledger-ink); background: var(--ledger-paper); border: 1px solid var(--ledger-line); border-radius: 6px; box-shadow: var(--ledger-shadow); }
...
.plan-facts div { min-width: 76px; padding: 8px 10px; background: #f6f7fc; border-left: 2px solid #bbc0fa; }
...
.progress-band { display: grid; grid-template-columns: auto minmax(80px, 1fr) auto; align-items: center; gap: 10px; margin: 16px 0 0; padding: 11px 12px; background: #f7f8fc; border: 1px solid #e4e8ef; }
...
.next-card { position: relative; display: grid; grid-template-columns: 7px minmax(0, 1fr) auto; gap: 15px; align-items: stretch; min-width: 0; padding: 15px; background: #f5f6ff; border: 1px solid #cdd2ff; border-left: 3px solid var(--ledger-indigo); }
.next-card.is-overdue { background: #fff5f4; border-color: #ebc5c3; border-left-color: var(--ledger-coral); }
.week-rail { display: grid; grid-template-rows: repeat(7, 1fr); gap: 2px; padding: 2px 0; }
.week-rail i { min-height: 4px; background: #bbc0fa; }
.is-overdue .week-rail i { background: #e0a19d; }
...
.complete-button { align-self: center; padding: 9px 15px; color: #fffefb; background: var(--ledger-indigo); border: 1px solid var(--ledger-indigo); }
...
.schedule-card { display: grid; grid-template-columns: 58px minmax(0, 1fr) auto; gap: 13px; align-items: center; min-width: 0; padding: 12px; background: #fffefb; border: 1px solid #e1e6ee; border-left: 3px solid #aeb8c8; scroll-margin-block: 18px; }
.schedule-card.is-overdue { border-left-color: var(--ledger-coral); }
.schedule-card.is-today { border-left-color: var(--ledger-indigo); background: #fafaff; }
.schedule-card.is-upcoming { border-left-color: #7782da; }
.schedule-card.is-focused { outline: 3px solid rgba(89, 100, 237, .32); outline-offset: 2px; }
...
.complete-button:focus-visible, .detail-button:focus-visible { outline: 3px solid rgba(89, 100, 237, .42); outline-offset: 3px; }
@keyframes loading-slide { 50% { transform: scaleX(.45); opacity: .55; } }
@media (max-width: 560px) { .study-week-board { padding: 16px; } .board-header { flex-direction: column; } .plan-facts { justify-content: flex-start; } .next-card { grid-template-columns: 6px minmax(0, 1fr); gap: 12px; } .complete-button { grid-column: 2; width: 100%; } .schedule-card { grid-template-columns: 52px minmax(0, 1fr); gap: 10px; } .detail-button { grid-column: 2; width: 100%; } .progress-band { grid-template-columns: auto minmax(0, 1fr); } .progress-percent { grid-column: 2; text-align: right; } }
@media (max-height: 480px) and (orientation: landscape) { .study-week-board { padding: 14px 16px; } .board-header { padding-bottom: 11px; } .next-action, .week-schedule { margin-top: 14px; } }
